...
os.environ['PATH'] = os.path.abspath(OPENSLIDE_PATH) + os.pathsep + original_path

try:
    # Import the C wrapper or the module that relies on the DLL
        ...
        for i, (x1, y1, x2, y2) in enumerate(self.regions):
            for x in range(math.ceil((x2 - x1) / 100)):
                for y in range(math.ceil((y2 - y1) / 100)):
                    regionData = np.asarray(
                        self.slide.read_region((x1 + x * 100, y1 + y * 100), 0, (100, 100)).convert("RGB")).ravel()

                    outputs = self.network.forward_pass(regionData)

                    if outputs[0] > 0.5 > outputs[1]:
                        self.detections += 1
                        self.detection_locations.append(((x1 + x * 100) / self.scan_x_scale, (y1 + y * 100) / self.scan_y_scale))

                    self.scan_count += 1
                    self.percent_complete = (self.scan_count / totalScansRequired) * 100
                    self.scan_location = [(x1 + x * 100) / self.scan_x_scale, (y1 + y * 100) / self.scan_y_scale]
                    self.relative_scan_location = [x * 100 / self.scan_x_scale, y * 100 / self.scan_x_scale]
...
class Server:
    def __init__(self):
        ...
        self.processor = Processor("../ZF.pyn") if os.path.exists("../ZF.pyn") else Processor(
            "../test_inverted.pyn")  # testing only
        self.running = True
